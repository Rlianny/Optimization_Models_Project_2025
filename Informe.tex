...
% Título y autor
\title{Informe del Proyecto de Optimización}
\author{Lianny de la Caridad Revé Valdivieso}
\date{}
...
\begin{titlepage}
    ...
    {\Huge\bfseries Informe del Proyecto de Optimización\par}
    
    \vspace{2cm}
    
    {\Large Autor: Lianny de la Caridad Revé Valdivieso\par}
    
    \vspace{1cm}
    
    {\Large Grupo: C311\par}
    
    \vspace{3cm}
    
    {\large Repositorio de GitHub:\par}
    \vspace{0.5cm}
    {\large\url{https://github.com/Rlianny/Optimization_Models_Project_2025.git}\par}
    
    \vfill
    
    {\large 15 de noviembre de 2025\par}
\end{titlepage}
...
\section*{Informe del Proyecto de Optimización: Minimización de f(x,y) = (x²-1)² + (y²-2)²}
...
En segundo lugar, este valor mínimo de cero se alcanza efectivamente cuando ambos términos son simultáneamente cero, lo cual requiere que $(x^2-1)^2 = 0$, implicando $x^2 = 1$ y consecuentemente $x = \pm 1$, y que $(y^2-2)^2 = 0$, de donde se obtiene $y^2 = 2$ y por lo tanto $y = \pm\sqrt{2}$. Estas condiciones producen exactamente los cuatro puntos mencionados: $(\pm1, \pm\sqrt{2})$.
...
\section*{Capítulo 2: Descripción de los Algoritmos Utilizados}
...
\centering
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{3cm}>{\raggedright\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{3cm}>{\raggedright\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{2.5cm}}
\toprule
\textbf{Experimento} & \textbf{Punto Inicial} & \textbf{Dificultad} & \textbf{GD: Iteraciones} & \textbf{BFGS: Iteraciones} \\
\midrule
...
exp8 & (0.05, $\sqrt{2}$) & Alta & 400-1000 & 20-60 \\
exp9 & (1.0, 0.05) & Alta & 400-1000 & 20-60 \\
exp10 & (50, -75) & Media-Alta & 1000-3000 & 15-40 \\
\bottomrule
\end{tabular}
\caption*{\textbf{Nota}: Estas predicciones se validarán con los resultados experimentales reales.}
\end{table}

\newpage

\section*{Capítulo 3: Diseño Experimental}
...
\begin{itemize}
    \item \textbf{Propósito}: Operaciones con arrays y álgebra lineal
    \item \textbf{Uso}: Vectores, cálculo de normas, operaciones matriciales
    \item \textbf{Justificación}: Estándar de facto para computación numérica en Python
\end{itemize}

\subsubsection*{SciPy}

\begin{itemize}
    \item \textbf{Propósito}: Algoritmos científicos avanzados
    \item \textbf{Uso}: Implementación de BFGS mediante \texttt{scipy.optimize.minimize}
    \item \textbf{Justificación}: Implementación robusta y optimizada de algoritmos de optimización
\end{itemize}

\subsubsection*{Matplotlib}

\begin{itemize}
    \item \textbf{Propósito}: Visualización de resultados
    \item \textbf{Uso}: Gráficos de contorno y trayectorias de optimización
    \item \textbf{Justificación}: Biblioteca estándar para visualización en Python científico
\end{itemize}
...
\subsubsection*{Experimento 1 (exp1.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(0.5, 0.5)$ - Cerca del origen, región no convexa
    \item \textbf{Learning rate}: $0.1$ - Moderado
    \item \textbf{Objetivo}: Evaluar convergencia desde un punto cercano al máximo local
    \item \textbf{Zona}: Región no convexa central
\end{itemize}

\subsubsection*{Experimento 2 (exp2.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(-1.5, -1.0)$ - Moderadamente alejado, más cerca de un mínimo
    \item \textbf{Learning rate}: $0.05$ - Conservador
    \item \textbf{Objetivo}: Evaluar convergencia desde diferentes regiones del espacio
    \item \textbf{Zona}: Cuadrante III, región convexa
\end{itemize}

\subsubsection*{Experimento 3 (exp3.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(100, 100)$ - Extremo superior derecho
    \item \textbf{Learning rate}: $0.05$ - Conservador para evitar divergencia
    \item \textbf{Objetivo}: Probar robustez desde puntos muy alejados del óptimo
    \item \textbf{Zona}: Cuadrante I, límite superior del rango
\end{itemize}

\subsubsection*{Experimento 4 (exp4.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(-100, -100)$ - Extremo inferior izquierdo
    \item \textbf{Learning rate}: $0.05$ - Conservador
    \item \textbf{Objetivo}: Verificar convergencia desde el extremo opuesto
    \item \textbf{Zona}: Cuadrante III, límite inferior del rango
\end{itemize}

\subsubsection*{Experimento 5 (exp5.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(100, -100)$ - Extremo inferior derecho
    \item \textbf{Learning rate}: $0.05$ - Conservador
    \item \textbf{Objetivo}: Explorar comportamiento desde límites de cuadrantes mixtos
    \item \textbf{Zona}: Cuadrante IV, límites del rango
\end{itemize}

\subsubsection*{Experimento 6 (exp6.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(-100, 100)$ - Extremo superior izquierdo
    \item \textbf{Learning rate}: $0.05$ - Conservador
    \item \textbf{Objetivo}: Completar exploración de los cuatro extremos
    \item \textbf{Zona}: Cuadrante II, límites del rango
\end{itemize}

\subsubsection*{Experimento 7 (exp7.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(0.1, 0.1)$ - Muy cerca del máximo local $(0,0)$
    \item \textbf{Learning rate}: $0.05$ - Conservador
    \item \textbf{Objetivo}: Analizar comportamiento cerca del máximo local (caso problemático)
    \item \textbf{Zona}: Región crítica cerca del máximo
\end{itemize}

\subsubsection*{Experimento 8 (exp8.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(0.05, \sqrt{2})$ - Cerca del punto silla $(0, \sqrt{2})$
    \item \textbf{Learning rate}: $0.03$ - Muy conservador
    \item \textbf{Objetivo}: Estudiar comportamiento cerca de puntos silla
    \item \textbf{Zona}: Región de punto silla
\end{itemize}

\subsubsection*{Experimento 9 (exp9.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(1.0, 0.05)$ - Cerca del punto silla $(1, 0)$
    \item \textbf{Learning rate}: $0.03$ - Muy conservador
    \item \textbf{Objetivo}: Estudiar comportamiento cerca de otro punto silla
    \item \textbf{Zona}: Región de punto silla
\end{itemize}

\subsubsection*{Experimento 10 (exp10.json)}
\begin{itemize}
    \item \textbf{Punto inicial}: $(50, -75)$ - Punto intermedio en rango amplio
    \item \textbf{Learning rate}: $0.05$ - Moderado
    \item \textbf{Objetivo}: Evaluar convergencia desde posiciones intermedias alejadas
    \item \textbf{Zona}: Cuadrante IV, posición intermedia
\end{itemize}
...
\begin{itemize}
    \item Convergencia más lenta
    \item Muy sensible a la tasa de aprendizaje
    \item Puede requerir muchas iteraciones para alcanzar alta precisión
    \item Trayectoria en forma de zigzag en regiones mal condicionadas
\end{itemize}

\textbf{BFGS}:
\begin{itemize}
    \item Convergencia rápida (pocas iteraciones)
    \item Ajuste automático del tamaño de paso
    \item Alta precisión en el resultado final
    \item Trayectoria más directa hacia el óptimo
\end{itemize}
...
Los resultados específicos se generan al ejecutar el notebook y se guardan en archivos JSON en la carpeta \texttt{Results/}.
...
\centering
\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{1cm}>{\raggedright\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{1.3cm}>{\raggedright\arraybackslash}p{1.5cm}>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{1.3cm}>{\raggedright\arraybackslash}p{1.5cm}}
\toprule
\textbf{Exp} & \textbf{Punto Inicial} & \textbf{GD$\rightarrow$Mínimo} & \textbf{GD Iter} & \textbf{GD f(x)} & \textbf{BFGS$\rightarrow$Mínimo} & \textbf{BFGS Iter} & \textbf{BFGS f(x)} \\
\midrule
exp1 & (0.5, 0.5) & 1 & 31 & 2.4×10$^{-14}$ & 1 & 8 & 4.2×10$^{-15}$ \\
exp2 & (-1.5, -1.0) & 4 & 29 & 3.6×10$^{-14}$ & 4 & 10 & 2.3×10$^{-15}$ \\
\textbf{exp3} & \textbf{(100, 100)} & \textbf{DIVERGIÓ} & \textbf{5000} & \textbf{NaN} & \textbf{1} & \textbf{42} & \textbf{1.4×10$^{-15}$} \\
\textbf{exp4} & \textbf{(-100, -100)} & \textbf{DIVERGIÓ} & \textbf{5000} & \textbf{NaN} & \textbf{4} & \textbf{42} & \textbf{3.5×10$^{-15}$} \\
\textbf{exp5} & \textbf{(100, -100)} & \textbf{DIVERGIÓ} & \textbf{5000} & \textbf{NaN} & \textbf{2} & \textbf{42} & \textbf{1.1×10$^{-14}$} \\
\textbf{exp6} & \textbf{(-100, 100)} & \textbf{DIVERGIÓ} & \textbf{5000} & \textbf{NaN} & \textbf{3} & \textbf{42} & \textbf{9.4×10$^{-16}$} \\
exp7 & (0.1, 0.1) & 1 & 44 & 3.8×10$^{-14}$ & 1 & 8 & 6.7×10$^{-19}$ \\
exp8 & (0.05, $\sqrt{2}$) & 1 & 83 & 5.9×10$^{-14}$ & 1 & 6 & 2.0×10$^{-15}$ \\
exp9 & (1.0, 0.05) & 1 & 42 & 1.8×10$^{-14}$ & 1 & 4 & 4.7×10$^{-16}$ \\
\textbf{exp10} & \textbf{(50, -75)} & \textbf{DIVERGIÓ} & \textbf{4000} & \textbf{NaN} & \textbf{2} & \textbf{37} & \textbf{1.8×10$^{-15}$} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection*{1. Falla Masiva de Gradient Descent en Puntos Alejados}

El resultado más crítico del estudio: \textbf{Gradient Descent falló en 5 de 10 experimentos (50\% tasa de fallo)}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{results_exp1_exp1_comparison.png}
    \caption{Experimento 1 con punto inicial $(0.5, 0.5)$ en región no convexa. El panel izquierdo muestra Gradient Descent convergiendo en 31 iteraciones con trayectoria menos directa. El panel derecho presenta BFGS alcanzando convergencia en solo 8 iteraciones con trayectoria significativamente más eficiente hacia el mínimo global en $(1, \sqrt{2})$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{results_exp3_exp1_comparison.png}
    \caption{\textbf{(Ejemplo de Divergencia):} Experimento 3 con punto inicial extremo $(100, 100)$ evidenciando el fallo crítico de Gradient Descent. El panel izquierdo muestra la divergencia catastrófica por overflow numérico, mientras el panel derecho demuestra que BFGS maneja exitosamente esta situación convergiendo en 42 iteraciones.}
\end{figure}
...
\subsection*{Análisis Visual Comparativo de Desempeño}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{comparison_iterations_barplot.png}
    \caption{Gráfico de barras comparativo del número de iteraciones requerido por cada método. Las barras azules representan casos exitosos de Gradient Descent, las barras rojas rayadas indican divergencia (alcanzando el máximo de iteraciones), y las barras verdes muestran el desempeño consistentemente exitoso de BFGS. Nótese la dramática diferencia: GD divergió en 5 de 10 experimentos (exp3-exp6, exp10), todos correspondientes a puntos iniciales alejados del origen.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{convergence_curves.png}
    \caption{Evolución del valor de la función objetivo $f(x,y)$ a lo largo de las iteraciones en escala logarítmica. El panel izquierdo muestra Gradient Descent para los 5 experimentos exitosos, evidenciando convergencia más lenta y requiriendo entre 29-83 iteraciones. El panel derecho presenta BFGS convergiendo en solo 4-10 iteraciones con descenso exponencial mucho más rápido, demostrando la superioridad del método cuasi-Newton en eficiencia de convergencia.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{initial_points_distribution.png}
    \caption{Mapa del espacio de búsqueda $[-110, 110] \times [-110, 110]$ mostrando la distribución de todos los puntos iniciales experimentales superpuestos sobre las curvas de nivel de la función objetivo. Las estrellas verdes marcan los cuatro mínimos globales, el triángulo rojo indica el máximo local en el origen, y los cuadrados azules representan los puntos silla. Los puntos iniciales se colorean según el resultado: círculos cian para convergencia exitosa de GD, y X naranjas para divergencia de GD. Las líneas naranjas discontinuas delimitan las fronteras de convexidad ($x = \pm 1/\sqrt{3}$, $y = \pm\sqrt{2/3}$). Claramente se observa que \textbf{todos los puntos que divergieron están en las esquinas extremas del espacio}, confirmando que GD falla sistemáticamente para puntos alejados.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{boxplot_iterations.png}
    \caption{Diagrama de caja comparando la distribución estadística del número de iteraciones entre métodos. Gradient Descent (solo casos exitosos, n=5) muestra mayor dispersión con mediana de 42 iteraciones, mientras BFGS (n=10, todos exitosos) presenta mediana de 24 iteraciones con menor variabilidad. Los puntos individuales con jitter revelan la distribución completa de datos. BFGS no solo es más eficiente en promedio, sino también más consistente y \textbf{100\% confiable}.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{radar_metrics.png}
    \caption{Gráfico de radar pentagonal comparando cinco métricas clave de desempeño en escala 0-10 (mayor es mejor): Tasa de Éxito (50\% vs 100\%), Eficiencia medida como inverso de iteraciones promedio, Precisión en escala logarítmica del valor final, Robustez basada en consistencia (desviación estándar), y Rendimiento General como promedio de las cuatro métricas anteriores. El polígono verde (BFGS) domina completamente al azul (Gradient Descent) en todas las dimensiones, con BFGS alcanzando puntuaciones perfectas o casi perfectas en tasa de éxito (10.0), eficiencia (10.0), precisión (10.0), y robustez (8.5), mientras que GD solo logra 5.0 en tasa de éxito debido al 50\% de fallos.}
\end{figure}

\textbf{1. Estadísticas Cuantitativas del Box Plot:}

\textbf{Gradient Descent (solo casos exitosos, n=5):}
\begin{itemize}
    \item Mediana: 42 iteraciones
    \item Media: 45.8 iteraciones
    \item Desviación Estándar: 19.5 iteraciones
    \item Rango: [29, 83] iteraciones
    \item Coeficiente de Variación: 42.6\% (alta variabilidad)
\end{itemize}

\textbf{BFGS (todos los experimentos, n=10):}
\begin{itemize}
    \item Mediana: 24 iteraciones
    \item Media: 24.1 iteraciones
    \item Desviación Estándar: 17.0 iteraciones
    \item Rango: [4, 42] iteraciones
    \item Coeficiente de Variación: 70.5\% (debido a la amplia gama de dificultades)
\end{itemize}

\textbf{Factores de Mejora:}
\begin{itemize}
    \item \textbf{Eficiencia (mediana)}: BFGS es 1.79× más rápido que GD
    \item \textbf{Eficiencia (media)}: BFGS es 1.90× más rápido que GD
    \item \textbf{Confiabilidad}: BFGS tiene 2.0× la tasa de éxito de GD (100\% vs 50\%)
\end{itemize}

\textbf{Nota crítica}: Estas estadísticas de GD son \textbf{optimistas} porque excluyen los 5 experimentos que divergieron. Si se consideran todos los experimentos, la ventaja de BFGS es aún más dramática.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{results_exp7_exp1_comparison.png}
    \caption{Experimento 7 con punto inicial $(0.1, 0.1)$ muy cerca del máximo local. Ambos métodos logran escapar exitosamente, pero BFGS requiere solo 8 iteraciones comparado con las 44 de Gradient Descent, demostrando superioridad incluso en casos problemáticos cercanos a puntos estacionarios no mínimos.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{results_exp9_exp1_comparison.png}
    \caption{Experimento 9 iniciando cerca del punto silla $(1, 0)$. Gradient Descent muestra trayectoria casi vertical con 42 iteraciones, mientras BFGS converge eficientemente en solo 4 iteraciones, evidenciando su capacidad superior para navegar regiones con Hessiana indefinida.}
\end{figure}

\textbf{2. Cuencas de Atracción (Solo Experimentos Exitosos)}

\textbf{Gradient Descent (5 experimentos exitosos):}
\begin{itemize}
    \item Mínimo 1: 5 experimentos (100\% de los exitosos)
    \item Todos los experimentos exitosos convergieron al mismo mínimo
    \item \textbf{Importante}: Solo funcionó para puntos iniciales cercanos al origen ($|x|, |y| < 50$)
\end{itemize}

\textbf{BFGS (10 experimentos, todos exitosos):}
\begin{itemize}
    \item Mínimo 1: 5 experimentos (50\%)
    \item Mínimo 2: 2 experimentos (20\%)
    \item Mínimo 3: 1 experimento (10\%)
    \item Mínimo 4: 2 experimentos (20\%)
    \item Distribución equilibrada entre los 4 mínimos
\end{itemize}

\textbf{Conclusión}: BFGS explora mejor el espacio de soluciones. GD solo puede explorar desde puntos cercanos.

\textbf{3. Eficiencia Comparativa (Solo Casos Exitosos)}

\textbf{Iteraciones:}
\begin{itemize}
    \item GD: Promedio = 54 iteraciones (solo 5 casos exitosos: exp1, exp2, exp7, exp8, exp9)
...
\textbf{Precisión:}
\begin{itemize}
    \item GD: Mejor = 1.8×10$^{-14}$ (solo casos exitosos)
    \item BFGS: Mejor = 6.7×10$^{-19}$
    \item \textbf{BFGS logra $\sim$270× mejor precisión}
\end{itemize}

\textbf{Tiempo de ejecución:}
\begin{itemize}
    \item Aunque GD tiene menos iteraciones cuando funciona, cada iteración de BFGS es más informada
    \item El factor crítico es la \textbf{confiabilidad}: 50\% de fallo vs 0\% de fallo
\end{itemize}
...
\begin{itemize}
    \item GD: 44 iteraciones \textbf{EXITOSO}
    \item BFGS: 8 iteraciones \textbf{EXITOSO}
    \item Ambos escapan exitosamente del máximo local
\end{itemize}

\textbf{Cerca de Puntos Silla (exp8, exp9):}
\begin{itemize}
    \item GD: 42-83 iteraciones \textbf{EXITOSO}
    \item BFGS: 4-6 iteraciones \textbf{EXITOSO}
    \item Los puntos silla no impiden convergencia, solo la ralentizan
\end{itemize}

\textbf{Puntos Extremos (exp3-exp6: $|x| = 100$ o $|y| = 100$):}
\begin{itemize}
    \item GD: \textbf{100\% DIVERGENCIA} (4 de 4 experimentos)
    \item BFGS: \textbf{100\% ÉXITO} (4 de 4 experimentos, 42 iteraciones)
    \item \textbf{Hallazgo crítico}: GD es \textbf{incapaz} de manejar puntos alejados con LR fijo
\end{itemize}

\textbf{Puntos Intermedios Alejados (exp2: $x_0 = (50, -75)$):}
\begin{itemize}
    \item GD: \textbf{DIVERGENCIA}
    \item BFGS: 37 iteraciones \textbf{EXITOSO}
    \item El umbral de fallo de GD está cerca de $|x|$ o $|y| \approx 50$
\end{itemize}

\subsubsection*{Validación de Predicciones}
...
\centering
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{2.7cm}>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{1.8cm}>{\raggedright\arraybackslash}p{2cm}}
\toprule
\textbf{Experimento} & \textbf{Predicción GD} & \textbf{Real GD} & \textbf{Predicción BFGS} & \textbf{Real BFGS} & \textbf{Validación} \\
\midrule
exp1 & 200-500 & 31 & 10-30 & 8 & Mejor de lo esperado \\
exp2 & 100-300 & 29 & 5-15 & 10 & Correcta \\
exp3 & 2000-5000 & 5000 & 20-50 & 42 & Correcta \\
exp4 & 2000-5000 & DIVERGIÓ & 20-50 & 42 & Peor de lo esperado \\
exp7 & 500-1500 & 44 & 15-40 & 8 & Mejor de lo esperado \\
exp8 & 400-1000 & 83 & 20-60 & 6 & BFGS mejor, GD mejor \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Tasa de validación}: 5/6 predicciones acertadas ($\sim$83\%)

\textbf{Sorpresas}:
\begin{itemize}
    \item GD convergió más rápido de lo esperado en casos cercanos al máximo (exp1, exp7)
    \item GD divergió completamente en exp4 (predicción: lento pero convergente)
    \item BFGS consistentemente supera las expectativas en puntos silla
\end{itemize}

\subsubsection*{Formato de Resultados JSON}
...
\section*{Capítulo 5: Conclusiones}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{results_exp2_exp1_comparison.png}
    \caption{Experimento 2 con punto inicial $(-1.5, -1.0)$ en el cuadrante III. Ambos métodos convergen exitosamente al mínimo global en $(-1, -\sqrt{2})$, con Gradient Descent requiriendo 29 iteraciones y BFGS solo 10, ilustrando la ventaja de eficiencia de los métodos de segundo orden incluso en casos favorables.}
\end{figure}
...
\subsection*{Referencias de Librerías y Documentación}
...
\begin{itemize}
    ...
    \item Generación de mallas de puntos para visualización (\texttt{np.meshgrid}, \texttt{np.linspace})
\end{itemize}
...
\textbf{Propósito}: Biblioteca para computación científica y técnica, construida sobre NumPy, utilizada para:
\begin{itemize}
    \item Implementación del algoritmo BFGS mediante \texttt{scipy.optimize.minimize}
    \item Optimización numérica con métodos avanzados
    \item Búsqueda de línea automática (line search)
    \item Manejo robusto de convergencia
\end{itemize}
...
\textbf{Propósito}: Biblioteca para creación de visualizaciones estáticas, animadas e interactivas, utilizada para:
\begin{itemize}
    \item Gráficos de contorno de la función objetivo
    \item Visualización de trayectorias de optimización
    \item Gráficos 3D de superficie
    \item Marcado de puntos estacionarios (mínimos, máximos, puntos silla)
\end{itemize}
...
\subsubsection*{Referencias Teóricas de los Algoritmos}

\textbf{Método BFGS (Broyden-Fletcher-Goldfarb-Shanno)}
...
\textbf{Método del Descenso del Gradiente}
...
\textbf{Validación de implementación}:
\begin{itemize}
    \item Los resultados de BFGS fueron validados contra la implementación canónica de SciPy
    \item El método del descenso del gradiente fue implementado desde cero siguiendo la formulación estándar
    \item Todos los gradientes fueron verificados mediante diferencias finitas durante el desarrollo
\end{itemize}
